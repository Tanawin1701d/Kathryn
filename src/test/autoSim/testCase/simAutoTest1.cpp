...
//            }
    ...
    public:
        ...
        sim1(testSimMod* md, int idx, const std::string& prefix, SimProxyBuildMode simProxyBuildMode):SimAutoInterface(idx,100,
                                                                  prefix + "simAutoResult"+std::toString(idx)+".vcd",
                                                                  prefix + "simAutoResult"+std::toString(idx)+".prof",
                                                                  simProxyBuildMode),
        _md(md)
        {}
        ...
        void simAssert() override{
            ...
            sim {
                ...
                testAndPrint("check End Val", (ull)_md->a,
                    32);
            };
            sim{
                ull testVal = 32;
    ...
    public:
        ...
        void start(std::string prefix, SimProxyBuildMode simProxyBuildMode) override{
            mMod(d, testSimMod, 1);
            startModelKathryn();
            sim1 simulator((testSimMod*) &d, _simId, prefix, simProxyBuildMode);
            simulator.simStart();
        }
