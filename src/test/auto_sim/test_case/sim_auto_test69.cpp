...
namespace kathryn{
    ...
    ////// simple pipeline
    class test_sim_mod69: public Module{
    public:
        m_reg (a, 32);
        m_wire(my_reset, 1);
        m_wire(my_start, 1);
        ...
        explicit test_sim_mod69(int x){}

        void flow() override{

            cloop(bid, 20){ intr_reset(my_reset); intr_start(my_start);
                a <<= (bid);
            }
    ...
    ///static std::string vcd_path = "/media/tanawin/tanawin1701e/project2/Kathryn/KOut/sim_auto_test69.vcd";
    ////static std::string profile_path = "/media/tanawin/tanawin1701e/project2/Kathryn/KOut/prof_auto_test69.vcd";
    ...
    public:
        ...
        sim69(test_sim_mod69* md, int idx, const std::string& prefix, SimProxyBuildMode sim_proxy_build_mode):SimAutoInterface(idx,
                                              200,
        ...
        void describe_con() override{
            ...
            con_next_cycle(1);
            for (int i = 1; i < 10; i++){
                test_and_print("check a REG is not set to" + std::to_string(i), ull(_md->a), i);
                con_next_cycle(1);
            }

            for (int i = 0; i < 4; i++){
                test_and_print("check a REG is not set to" + std::to_string(i), ull(_md->a), 9);
                con_next_cycle(1);
            }
            for (int i = 0; i < 5; i++){
                test_and_print("check a REG is not set to" + std::to_string(i), ull(_md->a), i);
    ...
    public:
        ...
        void start(std::string prefix, SimProxyBuildMode sim_proxy_build_mode) override{
            m_mod(d, test_sim_mod69, 1);
            start_model_kathryn();
            sim69 simulator((test_sim_mod69*) &d, _simId, prefix, sim_proxy_build_mode);
            simulator.sim_start();
        }
