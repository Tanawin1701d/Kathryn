...
impl HcpBaseVb for Val {
    ...
    /// Full declaration line: `wire [N-1:0] NAME = VALUE;` — a net declaration
    /// assignment (implicit continuous assign) so `always @(*)` blocks that read
    /// a constant Val get a real net in their sensitivity list and actually fire
    /// (a `localparam` is not a net, so a constant-only @(*) would never trigger).
