...
// lives in the host (`reduce_run`); this file wires it to Python: the `karray_dynamic_reduce_get`
...
use super::karray_util_py::karray_err_to_py;
use crate::model::common::identifier::Identifiable;
use crate::model::complex_hardware::common::ccp_ident::CcpIdent;
use crate::model::complex_hardware::karray::{reduce_mux, reduce_pack, reduce_run, NamedHcp, DynRdReduceDim, ReduceEnv};
use crate::model::hw_component::common::hcp_ident::HcpIdent;

#[pymethods]
impl PyModelArena {
    // Generic callback-driven reduce (winner get). The ALGORITHM lives in the host
    ...
    fn karray_dynamic_reduce_get(
        ...
        let dim_sels: Vec<DynRdReduceDim> = dims.into_iter()
            .map(|d| match d { Some(i) => DynRdReduceDim::Pin(i), None => DynRdReduceDim::Fold })
            .collect();
        ...
        let (shape, field_idxs, result_name) = {
            ...
            let (shape, field_idxs) = prepared.map_err(karray_err_to_py)?;
            (shape, field_idxs, name)
        };
