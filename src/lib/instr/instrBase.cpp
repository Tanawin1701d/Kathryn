...
namespace kathryn{
    ...
    _isSrc(isSrc),
    data (mOprReg("regData"  + std::toString(regNo) + (isSrc ? "Src": "Des"), archSize)),
    idx  (mOprReg("regIdx"   + std::toString(regNo) + (isSrc ? "Src": "Des"), idxSize)),
    valid(mOprReg("regValid" + std::toString(regNo) + (isSrc ? "Src": "Des"), 1)){
    ...
    _set(mOprReg("mop_" + mopName, 1)){
        ...
        for(const std::string& uopName: uopNames){
            ...
            _uopSets.pushBack(&mOprReg("uop_" + uopName, 1));
    ...
    InstrRepo::seperateMopByopcode(std::vector<MasterRule*>& msrs, int bitIdx){
        ...
        for (MasterRule* msr: msrs){  //// msr master rule
            ...
            switch(decVal){
            case '0' : {mopWithZero.pushBack(msr); break;}
            case '1' : {mopWithOne .pushBack(msr); break;}
    ...
    void InstrRepo::addMop(const MOP_META& mopMeta){
        ...
        mopMetas.pushBack(mopMeta);
        _amtMopType++;
    }

    MasterRule& InstrRepo::addDecRule(const std::string& workOnMopName,
                               const std::string& rule){
        masterRules.emplaceBack(this, workOnMopName, rule);
    ...
    void InstrRepo::declareHw(){
        ...
        for (int i = 0; i < _amtSrcOpr; i++){
            srcOprs.pushBack(new OPR_HW(
                OPR_WIDTH,
                log2Ceil(OPR_WIDTH),
                i, true));
        }
        for (int i = 0; i < _amtDesOpr; i++){
            desOprs.pushBack(new OPR_HW(
                OPR_WIDTH,
                log2Ceil(OPR_WIDTH),
                i, false));
        }
