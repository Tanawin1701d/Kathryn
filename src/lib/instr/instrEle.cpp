...
namespace kathryn{
    ...
    */
    ...
    void RegIdxAsm::doAsm(){
        ...
        auto slicedRegFileIdx = _master->getInstrOprPtr()->doSlice(_srcSlice);
    ...
    void ImmAsm::addImmMeta(const token& tk){
        ...
        immSlicer.pushBack({
            ...
            tk.sl,
            fillSl,
            _master->getInstrOprPtr()->doSlice(tk.sl)
        });
    ...
    void ImmAsm::doAsm(){
        ...
        for(int idx = 0; idx < immSlicer.size(); idx++){
            ...
            if (curStartIdx != iterImm.desSlice.start){
                arrangedOpr.pushBack({true,{-1,-1}, {curStartIdx, iterImm.desSlice.start}, nullptr});
            }
            arrangedOpr.pushBack(iterImm);
        ...
        if (curStartIdx != _master->getOprSize()){
            arrangedOpr.pushBack({true, {-1,-1}, {curStartIdx, _master->getOprSize()}, nullptr});
        }
    ...
    Operable* MatchSlaveVal::getActCondPtr(
    ...
    Operable* instr
        ...
        for (int matchIdx = 0; matchIdx < _matchRule._tokens.size(); matchIdx++){
            ...
            activateConditions.pushBack( &((*slicedInstr) == matchValue));
        }
    ...
    _immAsm(_master){
        ...
        for (const token& tk: _masterTokens._tokens){
            ...
            if (strVecTk.size() == 1){
                /////// it is op
                _opcodeTokens.pushBack(tk); ////// the main op decoder <0100011>
                continue;
            }
            //// case REG
            if(strVecTk[TOKEN_DEC_TYPE_IDX][0] == TOKEN_ASM_TYPE_REG_IDX){
                RegIdxAsm worker(_master,tk);
                if (worker.isRead){_srcRegAsms.pushBack(worker);}
                else              {_desRegAsms.pushBack(worker);}
                continue;
            }
            //// case IMM
    ...
    MasterRule& MasterRule::ad(const std::string& slaveRule,
        ...
        for(const auto& match : slaveMatch){
            ...
            _slaveRules.emplaceBack(slaveRule, match.matchVal, match.effUop);
        }
        return *this;
    }
        ...
        for(const auto&[uopName,uopIdx]: opHw.uopMapIdx){
            ...
            for(auto slaveRule:  _slaveRules){
                if (slaveRule._effUopName == uopName){
                    activateCons.pushBack(
                        slaveRule.getActCondPtr(_slaveTokenMap, _master->getInstrOprPtr()));
                }
            }
