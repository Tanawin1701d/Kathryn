...
namespace kathryn{
    ...
    {
        SimController* simCtrl = getSimControllerPtr();
    ...
    void SimInterface::simStartConSim(){
        ...
        conThread = std::makeUnique<std::thread>(&SimInterface::describeConWrapper,this);
    ...
    void SimInterface::trig(Operable& opr1, std::function<void()> callback){
        _traceEvents.emplaceBack(opr1, callback);
    }

    void SimInterface::trig(Operable& opr1, SIM_INTERFACE_EVENT event){
        switch (event){
            case EXIT_SIM:{
                _traceEvents.emplaceBack(opr1, [](){
    ...
    void SimInterface::testAndPrint(const std::string& testName, ValRepBase& simValLhs, ValRepBase& testValRhs) {
        ...
        if (simValLhs.getVal() == testValRhs.getVal()){
            ...
            std::cout << TC_RED << testName << " fail expect: "
                      << std::toString(testValRhs.getVal()) << "  got : "
                      << std::toString(simValLhs.getVal())  << TC_DEF << std::endl;
        }
    }
    ...
    void SimInterface::createModelSimEvent(){
        ...
        ////////// add to event queue
        getSimControllerPtr()->addEvent(_modelSimEvent);
