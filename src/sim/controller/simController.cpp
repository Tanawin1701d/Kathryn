...
namespace kathryn{
    ...
    void SimController::start() {
        ...
        while ( (!eventQ.isEmpty()) &&
                (eventQ.getNextEventPtr()->getCurCycle() <= _limitCycle)
        ){

            EventBase* nextEvent = eventQ.getNextEventPtr();
            ...
            while ((!eventQ.isEmpty()) && (eventQ.getNextEventPtr()->getCurCycle() == _curCycle)){
                _curCycleEvents.pushBack(eventQ.getNextEventPtr());
                eventQ.popEvent();
            }

            CYCLE amtUserLimit     = (_amtLrLimUser == nullptr)? INT64_MAX: *_amtLrLimUser;
            CYCLE nextEventOccurAt = eventQ.isEmpty()          ? INT64_MAX: eventQ.getNextEventPtr()->getCurCycle();
            /**
    ...
    SimController*getSimControllerPtr(){
