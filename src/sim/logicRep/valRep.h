...
namespace kathryn{
    ...
    public:
        ...
        void setVar(ull x) const{
            if (_byteSize == 1){
                (*staticCast<uint8_t*>(_val)) = (uint8_t)(x);
            }else if (_byteSize == 2){
                (*staticCast<uint16_t*>(_val)) = (uint16_t)(x);
            }else if (_byteSize == 4){
                (*staticCast<uint32_t*>(_val)) = (uint32_t)(x);
            }else if (_byteSize == 8){
                if (_continLength == -1){
                    (*staticCast<uint64_t*>(_val)) = (uint64_t)(x);
                }else if (_continLength > 0){
                    (*staticCast<uint64_t*>(_val)) = (uint64_t)(x);
                    for(int i = 1; i < _continLength; i++){
                        *(staticCast<uint64_t*>(_val) + i) = (uint64_t)(0);
                    }
                }else{
                    assert(false);
                }
        ...
        void setVarArr(const ull inputValue, ull size){
            ...
            if (_continLength > 0){
                ...
                if (_byteSize == 1){
                    std::fill(
                        staticCast<uint8_t*>(_val),
                        staticCast<uint8_t*>(_val) + size,
                        staticCast<uint8_t>(inputValue));
                }else if (_byteSize == 2){
                    std::fill(
                        staticCast<uint16_t*>(_val),
                        staticCast<uint16_t*>(_val) + size,
                        staticCast<uint16_t>(inputValue));
                }else if (_byteSize == 4){
                    std::fill(
                        staticCast<uint32_t*>(_val),
                        staticCast<uint32_t*>(_val) + size,
                        staticCast<uint32_t>(inputValue));

                }else if (_byteSize == 8){
                    std::fill(
                        staticCast<uint64_t*>(_val),
                        staticCast<uint64_t*>(_val) + size,
                        staticCast<uint64_t>(inputValue));
        ...
        void setLargeVar(std::vector<ull> x) const{
            ...
            if (_continLength > 0){
                for(int i = 0; i < _continLength; i++){
                    *(staticCast<uint64_t*>(_val) + i) = x[i];
                }
            }
        }
        ...
        ull getVal()const{
            ...
            if (_byteSize == 1){
                return *staticCast<uint8_t*>(_val);
            }
            if (_byteSize == 2){
                return *staticCast<uint16_t*>(_val);
            }
            if (_byteSize == 4){
                return *staticCast<uint32_t*>(_val);
            }
            if (_byteSize == 8){
                if (_continLength >= 0){
                    //// warning system should be declared here.
                }
                return *staticCast<uint64_t*>(_val);
            }
            assert(false);
        }
        ...
        std::vector<ull> getLargeVal()const{
            ...
            for (int idx = 0; idx < _continLength; idx++){
                result.pushBack(*(staticCast<uint64_t*>(_val) + idx));
            }
            return result;
        }
        ...
        ValRepBase operator [] (ull idx){
            if (_byteSize == 1){
                return {_byteSize, staticCast<uint8_t*>(_val) + idx};
            }
            if (_byteSize == 2){
                return {_byteSize, staticCast<uint16_t*>(_val) + idx};
            }
            if (_byteSize == 4){
                return {_byteSize, staticCast<uint32_t*>(_val) + idx};
            }
            if (_byteSize == 8){
                if (_continLength == -1){
                    return {_byteSize, staticCast<uint64_t*>(_val) + idx};
                }
                if (_continLength > 0){
                    return {_byteSize, staticCast<uint64_t*>(_val) + (_continLength * idx)};
                }
            }
            assert(false);
        }
        ...
        void fillZero(int startBit) const{
            ...
            case 1 :{
                ...
                (*staticCast<uint8_t*> (_val)) &= maxBit; break;
            }
            case 2 :{
                uint16_t maxBit = (1 << startBit) - 1;
                (*staticCast<uint16_t*> (_val)) &= maxBit; break;
            }
            case 4 :{
                uint32_t maxBit = (1 << startBit) - 1;
                (*staticCast<uint32_t*> (_val)) &= maxBit; break;
            }
            case 8 :{
                if (_continLength < 0){
                    uint64_t maxBit = (((ull)1) << startBit) - 1;
                    (*staticCast<uint64_t*> (_val)) &= maxBit;
                    break;
                }
                assert(_continLength != 0);
                ...
                uint64_t* dayta = staticCast<uint64_t*>(_val);
        ...
        ValRepBase operator >> (int amt){
            ...
            switch (_byteSize){
                case 1 : {*staticCast<uint8_t* >(newValRep._val) >> amt; break;}
                case 2 : {*staticCast<uint16_t*>(newValRep._val) >> amt; break;}
                case 4 : {*staticCast<uint32_t*>(newValRep._val) >> amt; break;}
                case 8 :{
                    if (_continLength < 0){
                        *staticCast<uint64_t*>(newValRep._val) >> amt; break;
                    }
                    assert(_continLength != 0);
                    ...
                    uint64_t* dayta = staticCast<uint64_t*>(newValRep._val);
    ...
    public:
        ...
        std::string toBiStr(){
            ...
            for(int i = arrSize - 1; i >= 0; i--){
                ...
                preRet += binaryRepresentation.toString();
            }
            return preRet;
        }
    ...
    //
    //     template<int slStart, int slStop>
    //     inline ull buildMask() const{
    //         int size = slStop - slStart;
    //         ull mask = (size == bitSizeOfUll) ? -1 : ((((ull)1) << size) - 1);
    //         return mask;
    //     }
    //
    //     template<int slStart, int slStop, int srcStart>
    //     inline  ValRep<slStop - slStart> sliceAndShift() const {
    //         assert((slStart>=0) && (slStart < slStop) && (slStop <= bitSizeOfUll));
    //         assert(srcStart < bitSizeOfUll);
    //         ull mask = buildMask<slStart, slStop>();
    //             mask = mask << slStart;
    //             mask &= _val;
    //         if (slStart <= srcStart){
    //             mask = mask << (srcStart - slStart);
    //         }else{
    //             mask = mask >>  (slStart - srcStart);
    //         }
    //         return ValRep<slStop - slStart>(mask);
    //     }
    //
    //     template<int slStart, int slStop>
    //     void updateOnSlice(const ValRep<slStop - slStart>& rhs){
    //         ull mask = buildMask<slStart, slStop>();
    //         mask = mask << slStart;
    ...
    //
    //
    //     template<int slStart, int slStop, int fixSize>
    //     inline  ValRep<fixSize> slice() const {
    //         assert(fixSize >= (slStop - slStart));
    //         assert((slStart>=0) && (slStart < slStop) && (slStop <= bitSizeOfUll));
    //         ull mask = buildMask<slStart, slStop>();
    //         return ValRep<fixSize>(mask & (_val >> slStart));
    //     }
    //
    //     template<int slStart, int slStop>
    //     inline  ValRep<slStop - slStart> slice() const {
    //         return slice<slStart, slStop, slStop - slStart>();
    //     }
    //
    //     bool getLogicValue(){
    //         return _val > 0;
