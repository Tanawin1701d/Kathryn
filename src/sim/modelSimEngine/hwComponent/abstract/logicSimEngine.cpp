...
namespace kathryn{
    ...
    void LogicSimEngine::createOpWithSoleCondition(CbBaseCxx& cb,
        ...
        for (UpdateEventBase* updateEvent : _asb->getUpdateMeta().getUpdateEventRef()){
            ...
            cb.addCm(getValRep().toString() + ": priority " + std::toString(updateEvent->getPriority()));
    ...
    std::string LogicSimEngine::genAssignAEqB(Slice desSlice, bool isDesTemp,
        ...
        std::string ret;
        std::string desAStr = std::toString(desSlice.start);
        std::string desBStr = std::toString(desSlice.stop);
        /////////////////////// clear old data
    ...
    ValR LogicSimEngine::genSlicedOprTo(Slice srcSlice, SIM_VALREP_TYPE_ALL desField){
        ...
        std::string aStr = std::toString(srcSlice.start);
        std::string bStr = std::toString(srcSlice.stop);
        return getValRep()
                .slice(srcSlice)
    ...
    void LogicSimEngine::proxyBuildInit(){
        ...
        for (UpdateEventBase* updateEvent : _asb->getUpdateMeta().getUpdateEventRef()){
            ...
            for (Operable* opr1 : depOpr){
                assert(opr1 != nullptr);
                dep.pushBack(opr1->getLogicSimEngineFromOprPtr());
            }
        }
    }
   ...
   // void LogicSimEngine::genOpWithChainCondition(CbBaseCxx& cb, const std::string& auxAssStr){
           ...
           //
           //  int idx = 0;
           //  int maxUpdateEvent = staticCast<int>(_asb->getUpdateMeta().size());
           ...
           //      updateEventGrp.pushBack(reversedUpdateEvents[idx]);
