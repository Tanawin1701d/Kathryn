...
namespace kathryn{
    ...
    struct SIM_VALREP_TYPE_I_ALL{
        ...
        subType(-1){
            if (staticCast<int>(type) == staticCast<int>(SVT_U64M)){
                subType = getArrSize(bitSize);
            }
        }
    ...
    struct ValR{
        ...
        std::string buildMask (const int size, const int start = 0) const{
            if(_valType.type == SVT_U64M){
                return "(" + SVT_toUnitType(_valType) + "(0).buildMask(" + std::toString(size) + "," + std::toString(start) + "))";
            }
            assert( (size + start) <= getSvtMaxBitSize(_valType));
            ull value = (size == bitSizeOfUll) ? (INT64_MAX << start) : (((1ULL << size) - 1) << start);
            return "(staticCast<" + SVT_toUnitType(_valType) +">(" + cvtNum2HexStr(value) + "))";
        ...
        std::string buildZero() const{
            ...
            return "(staticCast<" + SVT_toUnitType(_valType) +">(0))";
        }
        ...
        ValR slice (Slice sl) const{
            ...
            if (sl.start == 0){
                ...
                return { _valType, _size,"(("+_data+">>"+std::toString(sl.start)+")&" + buildMask(targetSize)+")"};
            }
        ...
        ValR shift(const int start) const{
            ...
            return {_valType, _size,"(" +_data + "<<" + std::toString(start) + ")"};
        }

        ValR castBase(SIM_VALREP_TYPE_ALL desVt, int size) const{

            if ((desVt.type == SVT_U64M)  && (_valType.type != SVT_U64M)){
                return {desVt, size, "UintX<" + std::toString(desVt.subType) + ">(" + _data + ")"};
            }

            return {desVt, size, "staticCast<" + SVT_toUnitType(desVt) + ">(" + _data + ")"};
        }
        ...
        std::string castToSignStr() const{
            ...
            return "staticCast<" + SVTI_toUnitType(svti) + ">(" + _data + ")";
        ...
        std::string buildVar(ull initVal) const{
            return SVT_toUnitType(_valType) + " " + _data + " = " + std::toString(initVal);
        }
        ...
        std::string buildMemVar(ull depthSize) const{
            return SVT_toUnitType(_valType) + " " + _data + "["+ std::toString(depthSize) + "]";
        }
