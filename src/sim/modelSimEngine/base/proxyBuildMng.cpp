...
namespace kathryn{
    ...
    void ProxyBuildMng::setStartModule(Module* startModule){
        ...
        moduleSimEngine = startModule->getSimEnginePtr();
    }
    ...
    void ProxyBuildMng::startWriteCallBackCheckAndRet(){
        ...
        for (int i = 0; i < callBackEvents->size(); i++ ){
            ...
            x.addSt(CALLBACK_VAR_ARR_NAME + "[" + CALLBACK_VAR_AMT + "] = " + std::toString(i) );
    ...
    void ProxyBuildMng::startWriteVcdDecVar(bool isUser){
        ...
        for (LogicSimEngine* mpb : dayta){
            if (mpb->isUserDeclare() == isUser){
                ...
                proxyfileWriter->addData(",");
                proxyfileWriter->addData("{" +
                    std::toString(mpb->getSize().start) + "," +
                    std::toString(mpb->getSize().stop) + "});\n");
            }
        }
        proxyfileWriter->addData("}\n");
    }
...
#endif
        int result = system(compileComand.cStr());
#ifdef MODELCOMPILEVB
        if (result == 0){
    ...
    ProxySimEventBase* ProxyBuildMng::loadAndGetProxy(){
        //////// open dynamic link
        _handle = dlopen(srcDynLoadPath.cStr(), RTLD_LAZY);
        const char* dlopenError = dlerror();
        if (dlopenError){
            std::cerr << "can't dynamic open " << dlopenError << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        if (dlsymError){
            std::cerr << "can't load symbol 'create': " << dlsymError << std::endl;
            dlclose(_handle);
            exit(EXIT_FAILURE);
        }
        /////// create object
