...
namespace kathryn{
    ...
    protected:
        ...
        std::unorderedMap<std::string, uint8_t*>    callBack8; //// for mem block use start point
        std::unorderedMap<std::string, uint16_t*>   callBack16; //// for mem block use start point
        std::unorderedMap<std::string, uint32_t*>   callBack32; //// for mem block use start point
        std::unorderedMap<std::string, uint64_t*>   callBack64; //// for mem block use start point
        std::unorderedMap<std::string, UintX_Base*> callBack64M;

        std::unorderedMap<std::string, uint64_t*> callBackPerf;
        std::unorderedMap<std::string, uint8_t*> callBackPerfCurbit;
    ...
    public:
        ...
        void setVcdWritePol    (MODULE_VCD_REC_POL vcdRecPol){VCD_REC_POL = vcdRecPol;}
        ...
        ValRepBase getVal    (const std::string& globalName){
            ...
            if (callBack64M.find(globalName) != callBack64M.end()){
                return {sizeof(uint64_t), callBack64M.find(globalName)->second->getDataBasePtr()};
            }
            assert(false);
