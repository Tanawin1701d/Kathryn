...
namespace kathryn{
    ...
    std::string SVT_toUnitType(SIM_VALREP_TYPE_ALL svt){
        ...
        if (svt.type == SVT_U64M){
            return "UintX<" + std::toString(svt.subType) + ">";
        }

        int idx = staticCast<int>(svt.type);
        assert(idx < staticCast<int>(SVT_CNT));
    ...
    std::string SVTI_toUnitType(SIM_VALREP_TYPE_I_ALL svt){
        if (staticCast<int>(svt.type) == staticCast<int>(SVT_U64M)){
    ...
    SIM_VALREP_TYPE_I_ALL cvtValRepType(SIM_VALREP_TYPE_ALL svt){
        ...
        SIM_VALREP_TYPE_INT convertedType = staticCast<SIM_VALREP_TYPE_INT>(svt.type);
