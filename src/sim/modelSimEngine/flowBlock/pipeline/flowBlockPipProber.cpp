...
namespace kathryn{
    ...
    bool PipSimProbe::isWaiting(){
        checkSimEngineReady();
        StateNode* waitNode = flowBlockPipBase->getWaitNodePtr();
        StateReg&  stateReg = *waitNode->_state_reg;
        bool isWaiting  = staticCast<bool>((ull)stateReg);
        return isWaiting;
    ...
    bool PipSimProbe::isExecuting(){
        checkSimEngineReady();
        FlowBlockBase* implicFb = flowBlockPipBase->getImplicitFlowBlockPtr();
        assert(implicFb != nullptr);
        return implicFb->getSimEnginePtr()->isBlockRunning();
    }
    ...
    bool ZyncSimProb::getPrepSendNodeStatus(){
        StateNode* preSendNode = flowBlockZyncBase->getPreSendNodePtr();
        StateReg&  stateReg = *preSendNode->_state_reg;
        bool isPrepSend = staticCast<bool>((ull)stateReg);
        return isPrepSend;
    }
