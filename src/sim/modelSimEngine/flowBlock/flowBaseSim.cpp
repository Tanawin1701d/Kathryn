...
namespace kathryn{
    ...
    void FlowBaseSimEngine::createOp(CbBaseCxx& cb){
        ...
        for (Node* sysNode : _flowBlockBase->getSysNodes()){
            ...
            for (CtrlFlowRegBase* stateReg : sysNode->getCycleRelatedReg()){
                if (stateReg != nullptr){
                    ValR stateRegRep = stateReg->getSimEnginePtr()->getValRep();
                    ValR checkRegRep =
        ...
        for (FlowBlockBase* subFb : _flowBlockBase->getSubBlocks()){
            FlowBaseSimEngine* subBlockSimEngine = subFb->getSimEnginePtr();
            cb.addSt( getVarNameCurStatus()
                .eq(getVarNameCurStatus() | subBlockSimEngine->getVarNameCurStatus())
                .toString()
            );
            for(FlowBlockBase* conOfSubFb: subFb->getConBlocks()){
                FlowBaseSimEngine* conOfSubFbSimEngine = conOfSubFb->getSimEnginePtr();
                cb.addSt( getVarNameCurStatus()
    ...
    void FlowBaseSimEngine::proxyRetInit(ProxySimEventBase* modelSimEvent){
        ...
        for (FlowBlockBase* subBlock : _flowBlockBase->getSubBlocks()){
            subBlock->getSimEnginePtr()->proxyRetInit(modelSimEvent);
        }
        ///////// conblock init
        for (FlowBlockBase* conBlock : _flowBlockBase->getConBlocks()){
            conBlock->getSimEnginePtr()->proxyRetInit(modelSimEvent);
        }
    }
