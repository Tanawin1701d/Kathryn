    ...
    struct DYNAMIC_FIELD{
        ...
        DYNAMIC_FIELD operator +(const DYNAMIC_FIELD& rhs){
            ...
            for (int i = 0; i < rhs._valueFieldNames.size(); i++){
                newD._valueFieldNames.pushBack(rhs._valueFieldNames[i]);
                newD._valueFieldSizes.pushBack(rhs._valueFieldSizes[i]);
            }
            return newD;
        }
        ...
        int findIdx(const std::string& name){
            ...
            sizeT findIdx = std::distance(_valueFieldNames.begin(), findIter);
            return findIdx;
        }
