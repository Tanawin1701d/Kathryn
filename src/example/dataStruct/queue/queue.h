...
namespace kathryn{
    ...
    struct Queue{
        ...
        explicit Queue(int wordAmt, DYNAMIC_FIELD fields):
        _fields(fields),
        WORD_SZ(_fields.sumFieldSize()),
        WORD_AMT(wordAmt),
        ADDR_WIDTH(log2Ceil(wordAmt)){
            assert(WORD_SZ > 0);
            assert(wordAmt  > 0);
        }
        ...
        getSimDebug(std::vector<int> subSizes){ ///// seperate site
            ...
            for (ull curRead = 0; curRead < curSizeInQueue; curRead++){
                ...
                for (auto rIter = subSizes.rbegin();
                    ...
                    currentResult.pushBack(std::toString(curSuffix));
                    readData = readData >> (*rIter);
                }

                result.pushBack(currentResult);
