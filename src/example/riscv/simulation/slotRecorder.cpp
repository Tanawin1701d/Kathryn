...
namespace kathryn::riscv{
    ...
    void SlotRecorder::writeFetchSlot(PipSimProbe* pipSimProbe) {
        ...
        _slotWriter->addSlotVal(RISC_FETCH, std::toString(ull(_riscv->fetch.parCheck)));

        ///if (_riscv->fetch.fetchBlock->getSimEnginePtr()->isBlockRunning()) {
        ...
        _slotWriter->addSlotVal(RISC_FETCH, "isFin " + std::toString(ull(_riscv->fetch.readFin)));
        _slotWriter->addSlotVal(RISC_FETCH, "isEn " + std::toString(ull(_riscv->fetch.readEn)));
        ...
        void SlotRecorder::writeDecodeSlot(PipSimProbe* pipSimProbe) {
            ...
            //_slotWriter->addSlotVal(RISC_FETCH, "parStart" + std::toString(ull(_riscv->decode.parCheck)));
            ...
            ull rawIntr  = ull(_riscv->coreData.ft.fetchInstr);
            ull pc       = ull(_riscv->coreData.ft.fetchPc);
            ull nextPc  = ull(_riscv->coreData.ft.fetchNextpc);
            ...
            //if (_riscv->decode.decodeBlk->getSimEnginePtr()->isBlockRunning()) {
            if (_riscv->decode.zyncSimProb.isExecuting()){
                ...
                _slotWriter->addSlotVal(RISC_DECODE, cvtNum2HexStr(nextPc));
            }else{
        ...
        void SlotRecorder::writeExecuteSlot(PipSimProbe* pipSimProbe) {
            ...
            _slotWriter->addSlotVal(RISC_EXECUTE, "readFin " + std::toString(ull(_riscv->execute.readFn)));
            _slotWriter->addSlotVal(RISC_EXECUTE, "readEn " + std::toString(ull(_riscv->execute.readEn)));
            //_slotWriter->addSlotVal(RISC_EXECUTE, "finLs " + std::toString(ull(_riscv->execute.testExit)));
            _slotWriter->addSlotVal(RISC_EXECUTE, "readAddr " + std::toString(ull(_riscv->execute.readAddr)));
            _slotWriter->addSlotVal(RISC_EXECUTE, "read1020 " + std::toString(ull(_riscv->memBlk._myMem.at(1020 >> 2))));
            _slotWriter->addSlotVal(RISC_EXECUTE, "read1024 " + std::toString(ull(_riscv->memBlk._myMem.at(1024 >> 2))));
            _slotWriter->addSlotVal(RISC_EXECUTE, "read1028 " + std::toString(ull(_riscv->memBlk._myMem.at(1028 >> 2))));
            _slotWriter->addSlotVal(RISC_EXECUTE, "resetSignal " + std::toString(ull(_riscv->execute.misPredic)));
        ...
        void SlotRecorder::writeWbSlot(PipSimProbe* pipSimProbe) {
            ...
            (RISC_WB, std::toString(ull(wbReg.data)));
        }
