...
namespace kathryn::o3{
    ...
    bool SimState::DispInstr::compare(const DispInstr& rhs) const{
        ...
        bool compareResult = true;
        compareResult &= checkAndPrintSimValueUll(immType  , rhs.immType , "dispatch", "immType  ");
        ...
        compareResult &= checkAndPrintSimValueUll(predAddr , rhs.predAddr, "dispatch", "predAddr ");
    ...
    bool SimState::RSV_BASE_ENTRY::compare(const RSV_BASE_ENTRY& rhs) const{
        ...
        compareResult &= checkAndPrintSimValueUll(aluOp , rhs.aluOp , name, "aluOp" );
    ...
    bool SimState::RSV_BRANCH_ENTRY::compare(const RSV_BRANCH_ENTRY& rhs) const{
        ...
        compareResult &= checkAndPrintSimValueUll(immBr, rhs.immBr, name, "immbr");
    ...
    bool SimState::RSV_MUL_ENTRY::compare(const RSV_MUL_ENTRY& rhs) const{
        ...
        compareResult &= checkAndPrintSimValueUll(selLohi   , rhs.selLohi   , name, "selLohi");
    ...
    bool SimState::COMMIT_ENTRY::compare(const COMMIT_ENTRY& rhs) const{
        ...
        compareResult &= checkAndPrintSimValueUll(wbFin   , rhs.wbFin   , "COMMIT entry " + std::toString(idx), "wbFin");
        if (wbFin == 0){
            return compareResult;
        }
        compareResult &= checkAndPrintSimValueUll(storeBit, rhs.storeBit, "COMMIT entry " + std::toString(idx), "storeBit");
        compareResult &= checkAndPrintSimValueUll(rdUse   , rhs.rdUse   , "COMMIT entry " + std::toString(idx), "rdUse");
        compareResult &= checkAndPrintSimValueUll(rdIdx   , rhs.rdIdx   , "COMMIT entry " + std::toString(idx), "rdIdx");
    ...
    bool SimState::STORE_BUF_ENTRY::compare(const STORE_BUF_ENTRY& rhs) const{
        ...
        return checkAndPrintSimValueUll (complete, rhs.complete, "STORE_BUF", "complete") &&
               ...
               checkAndPrintSimValueUll (memAddr, rhs.memAddr, "STORE_BUF", "memAddr") &&
               checkAndPrintSimValueUll (memData, rhs.memData, "STORE_BUF", "memData");
    }
    ...
    bool  SimState::MPFT_STATE::compare(const MPFT_STATE& rhs) const{
        ...
        for (int idx = 0; idx < SPECTAG_LEN; idx++){
                compareResult &= checkAndPrintSimValueBool(valids[idx], rhs.valids[idx], "MPFT", "valids" + std::toString(idx));
            if (valids[idx]){
                for (int fixIdx = 0; fixIdx < SPECTAG_LEN; fixIdx++){
                    compareResult &= checkAndPrintSimValueBool(fixTable[idx][fixIdx], rhs.fixTable[idx][fixIdx],
                                                               "MPFT", "fixTable idx: " + std::toString(idx) +
                                                               " fixIdx: " + std::toString(fixIdx));
                }
            }
    ...
    bool SimState::RRF_STATE::compare(const RRF_STATE& rhs, ull comPtr) const{
        ...
        for (ull idx = comPtr; idx != reqPtr; idx = (idx+1)%RRF_NUM){ ///// turn around check
            ...
            compareResult &= checkAndPrintSimValueBool(busy[idx], rhs.busy[idx], "RRF", "valids" + std::toString(idx));
            if (busy[idx]){
                compareResult &= checkAndPrintSimValueUll(data[idx], rhs.data[idx],
                                                           "RRF", "phyIdx: " + std::toString(idx));
            }
        }
    ...
    bool SimState::compare(SimState& rhs) const{
        ...
        compareResult &= execAlu1.compare(rhs.execAlu1);
        compareResult &= execAlu2.compare(rhs.execAlu2);
        compareResult &= execMul.compare(rhs.execMul);
        compareResult &= execBranch.compare(rhs.execBranch);
        compareResult &= execLdst.compare(rhs.execLdst);
