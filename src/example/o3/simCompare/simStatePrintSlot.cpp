...
namespace kathryn::o3{
    ...
    void writeIssue(const std::string& stageName,
        ...
        if (isOhIdx){
            writer.addSlotVal(RPS_ISSUE, "issue idx: " + std::toString(log2(issueIdx)));
        }else{
            writer.addSlotVal(RPS_ISSUE, "issue idx: " + std::toString(issueIdx));
    ...
    void SimState::Decode::printSlot(SlotWriterBase& writer){
        ...
        writer.addSlotVal(RPS_DECODE, "gennable "+ std::toString(isGenable));
        writer.addSlotVal(RPS_DECODE, "PC" + cvtNum2HexStr(pc));
        writer.addSlotVal(RPS_DECODE, "NPC" + cvtNum2HexStr(npc));

        for (int i = 1; i <= 2; i++){
            writer.addSlotVal(RPS_DECODE, "- DEC " + std::toString(i) + "-");
            ull simInvalid = (i == 1) ? 0     : invalid2;
            ull simInstr   = (i == 1) ? inst1 : inst2;

            if (simInvalid){
                writer.addSlotVal(RPS_DECODE, "notValid");
            }else{
                std::string decStr = "valid OP: ";
                decStr += translateInstrToOpCode(simInstr);
    ...
    void SimState::DispInstr::printSlot(SlotWriterBase& writer){
        ...
        if (invalid){
            ...
            writer.addSlotVal(RPS_DISPATCH, lookupImmType(immType));
            writer.addSlotVal(RPS_DISPATCH, "ALU: " + translateAluOpToOp(aluOp));

            writer.addSlotVal(RPS_DISPATCH, "isBr: " + std::toString(isBranch) +
                                            "/isSp: " + std::toString(spec) +
                                            "/spTag: " + cvtNum2BinStr(specTag));
            writer.addSlotVal(RPS_DISPATCH, "nextPc_" + cvtNum2HexStr(predAddr));

            std::string rdUsage = (rdUse != 0) ? "(USE)" : "(UNUSED)";
            writer.addSlotVal(RPS_DISPATCH, "RD: "+ rdUsage + " /ArchIdx: " +  std::toString(rdIdx));

            std::string r1Usage = (rsUse_1 != 0) ? "(USE)" : "(UNUSED)";
            std::string selStr = lookupSrcASel(rsSel_1);
            writer.addSlotVal(RPS_DISPATCH, "RS1: " + r1Usage + " /IDX:" + std::toString(rsIdx_1) +
                                    " /SEL:" + selStr);

            std::string r2Usage = (rsUse_2 != 0) ? "(USE)" : "(UNUSED)";
            std::string selStr2 = lookupSrcBSel(rsSel_2);
            writer.addSlotVal(RPS_DISPATCH, "RS2: " + r2Usage + " /IDX:" + std::toString(rsIdx_2) +
                                    " /SEL:" + selStr2);
        }
    ...
    void SimState::Dispatch::printSlot(SlotWriterBase& writer){
        ...
        writer.addSlotVal(RPS_DISPATCH, "aluRsvAble: " + std::toString(isAluRsvAllocatable));
        writer.addSlotVal(RPS_DISPATCH, "brRsvAble: " + std::toString(isBranchRsvAllocatable));
        writer.addSlotVal(RPS_DISPATCH, "isRenam: " + std::toString(isRenamable));
    ...
    void SimState::RSV_BASE_ENTRY::printSlot(SlotWriterBase& writer, REC_PIP_STAGE rps){
        ...
        if (ready){
            ...
            result0 += "W:";
            if (!(valid1 == 1)){
                result0 += std::toString(src1);
            }
            if (!(valid2 == 1)){
                result0 += "-";
                result0 += std::toString(src2);
            }
        }
        ...
        std::string result1;
        result1 += "spec: " + std::toString(specBit) + " spt:" + cvtNum2BinStr(spectag);
    ...
    void SimState::RSV_BASE_ENTRY::printDetailedSlot(SlotWriterBase& writer,
        ...
        writer.addSlotVal(rps, "ALU Op: " + translateAluOpToOp(aluOp) +
                               "/Spec: " + std::toString(specBit) +
                               "/SpecTag: " + cvtNum2BinStr(spectag));
        std::string sim_isRdUsed = dstval ? "(USE)" : "(UNUSED)";
        if (dstval){
            ull archIdx = commitState.comEntries[rrftag].rdIdx;
            writer.addSlotVal(rps, "RD phy: " + std::toString(rrftag) + " arch: " + std::toString(archIdx));
        }else{
            writer.addSlotVal(rps, "RD phy: " + std::toString(rrftag) + " arch(UNUSED)");
        }
    ...
    void SimState::RSV_BRANCH_ENTRY::printDetailedSlot(SlotWriterBase& writer,
        ...
        std::string result0 = "IMMBR: " + cvtNum2HexStr(immBr) + "/ PADDR: " + cvtNum2HexStr(praddr);
        writer.addSlotVal(rps, result0);
    ...
    void SimState::RSV_MUL_ENTRY::printDetailedSlot(SlotWriterBase& writer,
        ...
        ////// master call
        RSV_BASE_ENTRY::printDetailedSlot(writer, rps, commitState);
        std::string result0 = "sign1: " + std::toString(src1_signed) +
                              " /sign2: " + std::toString(src2_signed) +
                              " /selHi: " + std::toString(selLohi);
        writer.addSlotVal(rps, result0);
    }
    ...
    void SimState::EXEC_LDST_STATE::printSlot(SlotWriterBase& writer, COMMIT_STATE& commitState){
        ...
        if (writeSlotIfRunning(RPS_EXECUTE, st2, writer)){
            writer.addSlotVal(RPS_EXECUTE, "rdUse:" + std::toString(rdUse) +
                                           " /spec:" + std::toString(spec) +
                                           " /specTag:" + cvtNum2BinStr(specTag));
            writer.addSlotVal(RPS_EXECUTE, "stbHit:" + std::toString(stBufHit) +
                                           "stbData:" + cvtNum2HexStr(stBufData));
            writer.addSlotVal(RPS_EXECUTE, "loadData: " + std::toString(loadData));
        }
    }
    ...
    void SimState::COMMIT_ENTRY::printSlot(SlotWriterBase& writer){
        ...
        std::string entryStr = "E:" + std::toString(idx);
        entryStr += "/fin:" + std::toString(wbFin);

        entryStr += "/rd:";
        if (rdUse){
            entryStr += std::toString(rdIdx);
        }else{
            entryStr += "-";
        }
    ...
    void SimState::COMMIT_STATE::printSlot(SlotWriterBase& writer){
        writer.addSlotVal(RPS_COMMIT, "comPtr:" + std::toString(comPtr));
        writer.addSlotVal(RPS_COMMIT, "com1Status:" + std::toString(com1Status));
        writer.addSlotVal(RPS_COMMIT, "com2Status:" + std::toString(com2Status));
        if (com1Status){
    ...
    void SimState::STORE_BUF_ENTRY::printSlot(SlotWriterBase& writer){
        ...
        std::string result0;
        result0 += "[" + std::toString(idx) + " ";
        result0 += "cpt:" + std::toString(complete);
        result0 += "sp:" + std::toString(spec) + " ";
        result0 += "sptg:" + cvtNum2BinStr(specTag) + " ";
        writer.addSlotVal(RPS_STBUF, result0);

        std::string result1;
        result1 += "addr:" + cvtNum2HexStr(memAddr) + " ";
        result1 += "data:" + cvtNum2HexStr(memData);
        writer.addSlotVal(RPS_STBUF, result1);
    }

    void SimState::STORE_BUF_STATE::printSlot(SlotWriterBase& writer){
        writer.addSlotVal(RPS_STBUF, "finPtr: " + std::toString(finPtr));
        writer.addSlotVal(RPS_STBUF, "comPtr: " + std::toString(comPtr));
        writer.addSlotVal(RPS_STBUF, "retPtr: " + std::toString(retPtr));
        for (int idx = 0; idx < STBUF_ENT_NUM; idx++){
            entries[idx].printSlot(writer);
        }
        writer.addSlotVal(RPS_STBUF, "nb1 " + std::toString(nb1));
        writer.addSlotVal(RPS_STBUF, "ne1 " + std::toString(ne1));
        writer.addSlotVal(RPS_STBUF, "nb0 " + std::toString(nb0));
        writer.addSlotVal(RPS_STBUF, "fullNext " + std::toString(fullNext));
        writer.addSlotVal(RPS_STBUF, "emptyNext " + std::toString(emptyNext));
    }
    ...
    void SimState::MPFT_STATE::printSlot(SlotWriterBase& writer){
        for (int spIdx = 0; spIdx < SPECTAG_LEN; spIdx++){
            std::string specTagStr = "v: " + std::toString(valids[spIdx]) + " sp: ";
            for (int idx = (SPECTAG_LEN - 1); idx >= 0; idx--){
                specTagStr += std::toString(fixTable[spIdx][idx]);
            }
    ...
    void SimState::TAGGEN_STATE::printSlot(SlotWriterBase& writer){
        ...
        writer.addSlotVal(RPS_DECODE, "branch depth" + std::toString(brdepth));
    ...
    void SimState::ARF_STATE::printSlot(SlotWriterBase& writer, BC bcPrev){
        ...
        for (int tableIdx = 0; tableIdx < (SPECTAG_LEN+1); tableIdx++){
            writer.addSlotVal(RPS_ARF, "TABLEIDX " + std::toString(tableIdx));
            std::string reNameVal;
            for (int rrfIdx = 0; rrfIdx < REG_NUM; rrfIdx++){
                if (busy[tableIdx][rrfIdx]){
                    reNameVal += std::toString(rename[tableIdx][rrfIdx]) + "|";
                }else{
                    reNameVal += "-|";
                }
    ...
    void SimState::RRF_STATE::printSlot(SlotWriterBase& writer){
        writer.addSlotVal(RPS_RRF, "freeNum " + std::toString(freenum));
        writer.addSlotVal(RPS_RRF, "reqPtr"   + std::toString(reqPtr));
        writer.addSlotVal(RPS_RRF, "nextCycle" + std::toString(nextRrfCycle));
        // for (int rrfIdx = 0; rrfIdx < REG_NUM; rrfIdx++){
        //     writer.addSlotVal(RPS_RRF, "RRFIDX " + std::toString(rrfIdx) +
        //                                 " /V: " + std::toString(busy[rrfIdx]) +
    ...
    void SimState::printSlotWindow(SlotWriterBase& writer){
        ...
        writeIssue("alu1", writer, stIssueAlu1  , idxIssueAlu1, true);
        writeIssue("alu2", writer, stIssueAlu2  , idxIssueAlu2, true);
        writeIssue("mul", writer, stIssueMul   , idxIssueMul  , true);
        writeIssue("branch", writer, stIssueBranch, idxIssueBranch, false);
        writeIssue("ldst", writer, stIssueLdst  , idxIssueLdst    , false);


        /////// execute unit

        execAlu1.printSlot(writer, rob);
        execAlu2.printSlot(writer, rob);
        execMul.printSlot(writer, rob);
        execBranch.printSlot(writer, rob, bcState);
        execLdst.printSlot(writer, rob);
