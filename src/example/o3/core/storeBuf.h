...
namespace kathryn::o3{
    ...
    struct StoreBuf{
        ...
        std::pair<opr&, opr&> searchNewest(opr& addr){
            ...
            /// find newst system.
             auto[result, binIdx] = _table.findMBO_BIDX(true, finPtr,
                 [&](RegSlot& lhs)->opr&{
                 return lhs(busy) & (lhs(memAddr) == addr);
             });
            return {result(busy) & (result(memAddr) == addr), daytas[binIdx]};
        }
        ...
        void flow(){ //// retire and do other thing
            ...
            lss.dmemRwaddr = retireSlot(memAddr);
            lss.dmemWdata = daytas[retPtr];

            zif (retireSlot(busy) & retireSlot(complete) & (~bc.isBrMissPred())){
                zif(lss.dmemWe = 1){ ///// try to make it equal to one if not update will not occur because memory hold them all  ///CTRL GROB
                    retPtr <<= (retPtr + 1);
