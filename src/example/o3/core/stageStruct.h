...
namespace kathryn::o3{
    ...
    struct LdStStage{
        mWire(dmemRdata, DATA_LEN);
        mWire(dmemWe , 1); ///CTRL GROB
        mWire(dmemRwaddr, ADDR_LEN); //// must mux with reading
        mWire(dmemWdata, DATA_LEN);
        RegSlot lsRes {smLdSt};
        SyncPip  sync2 {"ldStLastSync"}; ///CTRL EXEC_LDST

        LdStStage(){
            dmemRdata  .asInputGlob ("dmemRdata");
            dmemWe     .asOutputGlob("dmemWe"); ///CTRL GROB
            dmemRwaddr .asOutputGlob("dmemRwaddr");
            dmemWdata  .asOutputGlob("dmemWdata");
    ...
    struct ByPassPool{
        ...
        ByPass& addByPassEle(){
            _bps.emplaceBack(new ByPass(_bps.size()));
            return **_bps.rbegin();
        }
    ...
    struct PipStage{
        ...
        SyncMeta syncDp    {"dispSync"}; ///CTRL DECODE
        SyncMeta syncRs    {"rsvSync"}; ///CTRL DISPATCH

        SyncMeta syncCm    {"commitSync"}; ///CTRL ROB
        ...
        void onMisPred(){
            ...
            syncDp.killSlave(true); ///CTRL DISPATCH
            syncCm.holdSlave();     ///CTRL ROB



            ////// kill the out-of-order exec Unit stage
            //syncEx1  .killIfTagMet(true, fixTag);
            //syncEx2  .killIfTagMet(true, fixTag);
            //syncMul  .killIfTagMet(true, fixTag);
        ...
        void onSucPred(){
            ...
            syncDp.holdMaster(); ///CTRL DISPATCH //// hold decode <-> dispatch to generate tag, but allowing system to enter decode state
            syncRs.holdMaster(); ///CTRL RSV_SHARED //// hold dispatch <-> reservation station
        }
