...
impl SequenceEle {
    ...
    fn finish_node(&self) -> NcpIdent {
        match self {
            Self::Basic    { state_node, .. } => state_node.expect("not generated"),
    ...
    fn add_depend_node(&self, src: NcpIdent, cond: Option<HcpIdent>, arena: &mut ModelArena) {
        match self {
            Self::Basic { state_node, .. } => {
                arena.add_depend_node_to_ncp(state_node.expect("not generated"), src, cond);
            }
            Self::SubBlock { node_wrap, .. } => {
                for entrance in node_wrap.as_ref().expect("not generated").get_entrance_nodes_i() {
    ...
    fn assign_entrance_nodes(&self, arena: &mut ModelArena, force_reassign: bool) {
        match self {
            Self::Basic { asm_node, state_node } => {
                arena.assign_ncp_node(state_node.expect("not generated"));
                arena.assign_asm_from_state_node(*asm_node);
            }
            Self::SubBlock { node_wrap, .. } => {
                if !force_reassign { return; }
                for entrance in node_wrap.as_ref().expect("not generated").get_entrance_nodes_i() {
...
impl SeqSchematic {
    ...
    pub fn add_node(&mut self, node: NcpIdent) {
        self.elements.push(SequenceEle::Basic { asm_node: node, state_node: None });
    }

    pub fn add_sub_block(&mut self, block: FlowBlockIdent) {
        self.elements.push(SequenceEle::SubBlock { block, node_wrap: None });
    }
