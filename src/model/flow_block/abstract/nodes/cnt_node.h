...
#include "node.h"
#include "model/flow_block/abstract/sp_reg/cnt_reg.h"
...
namespace kathryn{
    ...
    struct CounterNode : Node{
        ...
        explicit CounterNode(int last_loop_cnt, CLOCK_MODE clock_mode):
                Node(COUNTER_NODE){

            assert(last_loop_cnt > 0);
             _counter = new CounterReg(last_loop_cnt);
        ...
        void make_user_reset_event() override{
            if(is_threre_int_reset()){
                _counter->make_user_rst_event(int_reset->get_exit_opr_ptr(), get_clock_mode());
            }
        }
        ...
        Operable*get_counter_ptr(){return _counter;}
        ...
        void assign() override{
            ...
            _counter->set_var_name(ident_name);
        }
