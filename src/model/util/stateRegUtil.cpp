    ...
    std::vector<Operable*> getDependState(Operable* dependOpr){
        ...
        switch (hwType){
            ...
            case TYPE_EXPRESSION: {
                auto *expr = dynamicCast<expression *>(ident);
                auto preResult0= getDependState(expr->getOperandAPtr());
                auto preResult1 = getDependState(expr->getOperandBPtr());
