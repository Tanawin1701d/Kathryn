...
namespace kathryn {
    ...
    T& _make(const std::string& typeName, const std::string& name, bool isUserDec,Args&&... args){
        staticAssert(std::isBaseOf<HwCompControllerItf, T>::value,
                "make model component must base on ModelController controllable"
                );
        staticAssert(std::isBaseOf<IdentBase, T>::value,
        ...
        objPtr->comFinal(); /** /* typicallly it is used only module and box*/
