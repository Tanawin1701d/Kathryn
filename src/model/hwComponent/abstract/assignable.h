...
namespace kathryn{
    ...
    * */
    ...
    protected:
        ...
        virtual Assignable*getAssignableFromAssignOprPtr() = 0;
    public:
        virtual RET_TYPE& operator <<= (Operable& b){
            Assignable* asb = getAssignableFromAssignOprPtr();
    ...
    protected:
        void setMaster(RET_TYPE* master){_master = master;}
        virtual Assignable*getAssignableFromAssignCallbackerPtr() = 0;
    public:
        virtual void callBackBlockAssignFromAgent(Operable& b, Slice absSliceOfHost){
            getAssignableFromAssignCallbackerPtr()->doBlockAsm(b, absSliceOfHost);
            assert(_master != nullptr);
        }
        virtual void callBackNonBlockAssignFromAgent(Operable& b, Slice absSliceOfHost){
            getAssignableFromAssignCallbackerPtr()->doNonBlockAsm(b, absSliceOfHost);
            assert(_master != nullptr);
        }
        virtual void      callBackBlockAssignFromAgent(Operable& srcOpr,
                                                       ...
                                                       Slice  absSrcSlice,
                                                       Slice  absDesSlice){
            getAssignableFromAssignCallbackerPtr()->doBlockAsm(srcOpr, resultMetaCollector, absSrcSlice, absDesSlice);
            assert(_master != nullptr);
        }
        virtual void      callBackNonBlockAssignFromAgent(Operable& srcOpr,
                                                          ...
                                                          Slice  absSrcSlice,
                                                          Slice  absDesSlice){
            getAssignableFromAssignCallbackerPtr()->doNonBlockAsm(srcOpr, resultMetaCollector, absSrcSlice, absDesSlice);
            assert(_master != nullptr);
        }
    };
