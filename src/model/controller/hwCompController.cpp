...
namespace kathryn{
     ...
     *
     * */
    void ModelController::onSpRegInit(CtrlFlowRegBase* ptr, SP_REG_TYPE spRegType) {
        assert(ptr != nullptr);
        ...
        /** debug value*/
        logMf(ptr,
              "[" + spRegTypeToStr(spRegType) + "] is initialized and set parent to "
    ...
    /** register handling*/
    void ModelController::onRegInit(Reg* ptr) {
        assert(ptr != nullptr);
    ...
    void ModelController::onRegUpdate(AsmNode* asmNode, Reg* srcReg){
        /**
        ...
        if(isTopFbBelongToTopModule()){
            auto fb = getTopFlowBlockBasePtr();
     ...
     *
     * */
    void ModelController::onWireInit(Wire* ptr){
        assert(ptr != nullptr);
        on_box_tryAddToBox(ptr, ptr);
        Module* targetModulePtr = getTopModulePtr();
        /** localize necessary destination*/
    ...
    void ModelController::onWireUpdate(AsmNode* asmNode, Wire* srcWire) {
        /**
         * please note that UpdateEvent should fill update value/ and slice
         * but it must let update condition and state as nullptr to let block fill
         * to it
    ...
    /** exprMetas*/
    void ModelController::onExpressionInit(expression* ptr) {
        assert(ptr != nullptr);
        on_box_tryAddToBox(ptr, ptr);
        Module* targetModulePtr = getTopModulePtr();
        /** localize necessary destination*/
    ...
    /** nest*/
    void ModelController::onNestInit(nest* ptr) {
        assert(ptr != nullptr);
        on_box_tryAddToBox(ptr, ptr);
        Module* targetModulePtr = getTopModulePtr();
        /** localize necessary destination*/
    ...
    void ModelController::onNestUpdate(AsmNode* asmNode, nest* srcNest) {
        /**
         * please note that UpdateEvent should fill update value/ and slice
         * but it must let update condition and state as nullptr to let block fill
         * to it
    ...
    /** value*/
    void ModelController::onValueInit(Val* ptr) {
        assert(ptr != nullptr);
        on_box_tryAddToBox(ptr, ptr);
        Module* targetModulePtr = getTopModulePtr();
        /** localize necessary destination*/
    ...
    /** box*/
    void ModelController::onBoxInit(Box* ptr) {
        assert(ptr != nullptr);
        Module* targetModulePtr = getTopModulePtr();
        /** localize necessary destination*/
    ...
    void ModelController::onBoxEndInit(Box* ptr){
    ...
    void ModelController::onBoxUpdate(AsmNode* asmNode, Box* srcBox) {
        /**
         * please note that UpdateEvent should fill update value/ and slice
         * but it must let update condition and state as nullptr to let block fill
         * to it
    ...
    void ModelController::onItfInit(ModelInterface* ptr){
    ...
    void ModelController::onModuleInitComponents(Module* ptr) {
    ...
    void ModelController::onModuleEndInitComponents(Module* ptr){
    ...
    void ModelController::onModuleFinal(Module* ptr) {
        assert(!moduleStack.empty() && ( moduleStack.top().md == ptr ));
