...
namespace kathryn{
    ...
    FlowBlockBase* ModelController::get_top_flow_block_base_ptr() {
        if (flow_block_stacks[FLOW_ST_BASE_STACK].empty()){
           return nullptr;
        }
        return flow_block_stacks[FLOW_ST_BASE_STACK].top();
    }
    ...
    FlowBlockBase* ModelController::get_top_flow_block_base_ptr(FLOW_STACK_TYPE flow_stack_type){
        ...
        if (flow_block_stacks[flow_stack_type].empty()){
            return nullptr;
        }
        return flow_block_stacks[flow_stack_type].top();
    }

    void ModelController::pop_flow_block(FlowBlockBase* fb){
        assert(!flow_block_stacks[FLOW_ST_BASE_STACK].empty());
        assert( flow_block_stacks[FLOW_ST_BASE_STACK].top() == fb);

        for (int st_idx: fb->get_sel_fb_stack()){
            assert(st_idx < FLOW_ST_CNT);
            assert(!flow_block_stacks[st_idx].empty());
            assert(flow_block_stacks[st_idx].top() == fb);
            log_mf(fb, "pop_flowBlock " + std::to_string(st_idx));
            flow_block_stacks[st_idx].pop();
        }
    }
    ...
    void ModelController::push_flow_block(FlowBlockBase* fb) {
        assert(fb != nullptr);
        for (int st_idx: fb->get_sel_fb_stack()){
            log_mf(fb, "push to stack " + std::to_string(st_idx));
            assert(st_idx < FLOW_ST_CNT);
            flow_block_stacks[st_idx].push(fb);
        }
    }
    ...
    void ModelController::detach_top_flow_block() {
        ...
        log_mf(top_fb, "on_detach_flowBlock");
        pop_flow_block(top_fb);
        //////top_fb->build_hw_component();


        /**get front node to inject the subblock*/
        FlowBlockBase* front_fb = get_top_flow_block_base_ptr();
        if (!is_top_fb_belong_to_top_module()){
            log_mf(top_fb, "add_flow_block to module");
            Module* parent_mod = get_top_module_ptr();
            parent_mod->add_flow_block(top_fb);
        }else if (top_fb->get_join_fb_pol() == FLOW_JO_CON_FLOW){
            /**it is consecutive block*/
            log_mf(top_fb, "add_flow_block to be con module");
            front_fb->add_con_flow_block(top_fb);
        }else if (top_fb->get_join_fb_pol() == FLOW_JO_SUB_FLOW){
            /**it is sub block*/
            log_mf(top_fb, "add_flow_block to be sub module");
            front_fb->add_sub_flow_block(top_fb);
        }else if (top_fb->get_join_fb_pol() == FLOW_JO_EXT_FLOW){
            log_mf(top_fb, "extract flowblock and give it to basic asm");
            std::vector<AsmNode*> extracted_asm_node = top_fb->extract();
            for (auto asm_node: extracted_asm_node){
                front_fb->add_element_in_flow_block(asm_node);
            }
            // for (auto basic_node: top_fb->get_basic_node()){
            //     front_fb->add_element_in_flow_block(basic_node);
            // }
    ...
    bool ModelController::is_all_flow_stack_empty(){
        ...
        for(const auto & flow_block_stack : flow_block_stacks){
    ...
    bool ModelController::is_flow_stack_empty(FLOW_STACK_TYPE flow_stack_type){
        ...
        return flow_block_stacks[flow_stack_type].empty();
    }


    bool ModelController::is_top_fb_belong_to_top_module(){
        assert(get_top_module_ptr() != nullptr);
        return (!flow_block_stacks[FLOW_ST_BASE_STACK].empty()) &&
                (flow_block_stacks[FLOW_ST_BASE_STACK].top()->get_module_parent_ptr() == get_top_module_ptr());
    ...
    void ModelController::on_attach_flowBlock(FlowBlockBase *fb) {
        ...
        log_mf(fb, "on_attach_flowBlock");
        assert(fb != nullptr);
        if (fb->get_purify_req()){
            log_mf(fb, "try purify stack");
    ...
    void ModelController::on_attachAndDetach_intrSignal(INT_TYPE int_type, Operable* sig) {
        assert(sig != nullptr);
        try_purify_flow_stack();
        mf_assert(is_top_fb_belong_to_top_module(), "There is no flow block to add intr signal");
        auto top_fb = get_top_flow_block_base_ptr();
        top_fb->add_int_signal(int_type, sig);
    ...
    //     try_purify_flow_stack();
    //     mf_assert(is_top_fb_belong_to_top_module(), "there is no flow block to get next ready signal");
    //     assert((!flow_block_stacks[FLOW_ST_PIP_WRAP].empty()) &&
    //            (flow_block_stacks[FLOW_ST_PIP_WRAP].top()->get_module_parent_ptr() == get_top_module_ptr())
    //            );
    //
    //     /*** get and gen signal*/
    //     auto pip_wrap_blk = flow_block_stacks[FLOW_ST_PIP_WRAP].top();
    //     assert(pip_wrap_blk->get_flow_type() == PIPE_WRAPPER);
    //     auto castedpip_wrap_blk = (FlowBlockPipeWrapper*) pip_wrap_blk;
    //     return castedpip_wrap_blk->get_next_pip_block_ready_signal();
    // }

    FLOW_BLOCK_TYPE ModelController::get_top_pattern_flow_block_type(){

        bool top_pattern_fb_belong_to_top_module =
                  (!flow_block_stacks[FLOW_ST_PATTERN_STACK].empty())
                && (flow_block_stacks[FLOW_ST_PATTERN_STACK].top()->get_module_parent_ptr() == get_top_module_ptr()
                );

        if (top_pattern_fb_belong_to_top_module){
            FlowBlockBase* fb = flow_block_stacks[FLOW_ST_PATTERN_STACK].top();
            assert(fb != nullptr);
