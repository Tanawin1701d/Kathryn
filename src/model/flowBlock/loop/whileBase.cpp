...
namespace kathryn{
    ...
    void FlowBlockWhile::buildHwComponent(){
        ...
        if (getFlowType() == CWHILE){
            ...
            conditionNode->setInternalIdent("cConNode" + std::toString(getGlobalId()));
        }else{////// SWHILE
            conditionNode = new StateNode(getClockMode());
            conditionNode->setInternalIdent("sConNode" + std::toString(getGlobalId()));
        ...
        conditionNode->addDependNode(subBlockNodeWrap->getExitNode(),
                                     subBlockNodeWrap->isThereForceExitNode()?
                                        ( &(~(*subBlockNodeWrap->getForceExitNodePtr()->getExitOprPtr())) ):
                                        nullptr
                                     );
        if(isThereIntStart()){
        ...
        if (subBlockNodeWrap->isThereForceExitNode()){
            exitNode->addDependNode(subBlockNodeWrap->getForceExitNodePtr(), nullptr);
        }
