...
namespace kathryn{
            ...
            purifiedCurCond(&(*purifyCondition(&cond)))
    {
        ...
        prevFalses.pushBack(&(!(*purifiedCurCond)));
    ...
    void FlowBlockZIF::addConFlowBlock(FlowBlockBase *fb){
        ...
        for (ZifClassAsm* slaveAsmMeta: subClassAsmMeta){
            ...
            if (!found){
                ...
                _assignMetas.pushBack(slaveAsmMeta);
            }
        }
        /** generate next prev false*/
        if (castedZelif->getPurifiedCurCondPtr() == nullptr){
            lastZelifDetected = true;
        }else{
            prevFalses.pushBack(
                    &((*prevFalse) & (!(*castedZelif->getPurifiedCurCondPtr())))
            );
        }
    ...
    std::vector<AsmNode*> FlowBlockZIF::extract(){
        ...
        for (ZifClassAsm* zifClassAsm: _assignMetas){
            result.pushBack(zifClassAsm->createAsmNode());
        }
        return result;
