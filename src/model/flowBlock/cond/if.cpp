...
namespace kathryn{
    ...
    FlowBlockBase(flowBlockType,
       {
        ...
        allCondes.pushBack(&cond);
        allPurifiedCondes.pushBack(purifyCondition(&cond));
    }
    ...
    void FlowBlockIf::addConFlowBlock(FlowBlockBase* conBlock){
        ...
        /// allStatement.pushBack(elifBlock->sumarizeBlock());
        if (elifBlock->getConditionPtr() != nullptr) {
            allCondes.pushBack(elifBlock->getConditionPtr());
            allPurifiedCondes.pushBack(
                    purifyCondition(elifBlock->getConditionPtr())
            );
        }
    ...
    void FlowBlockIf::buildHwComponent() {
        ...
        allStatement.pushBack(_subBlocks[0]->sumarizeBlock());
        for (auto conFlowBlock: _conBlocks){
            allStatement.pushBack(conFlowBlock->sumarizeBlock());
        }
        ...
        if (getFlowType() == CIF){
            ...
            condNode->setInternalIdent("cifNode" + std::toString(getGlobalId()));
        }else if(getFlowType() == SIF){
            condNode = new StateNode(getClockMode());
            condNode->setInternalIdent("sifNode" + std::toString(getGlobalId()));
        ...
        addSysNode(exitNode);
        exitNode->setInternalIdent("ifExitNode" + std::toString(getGlobalId()));
    ...
    void FlowBlockIf::addMdLog(MdLogVal *mdLogVal) {
        ...
        for (auto sb : _subBlocks){
            std::string subBlockHeaderDebug = "----> subblock " + std::toString(cnt) + " condition ";
