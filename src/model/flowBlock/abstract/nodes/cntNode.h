...
namespace kathryn{
    ...
    struct CounterNode : Node{
        ...
        void makeUserResetEvent() override{
            if(isThrereIntReset()){
                _counter->makeUserRstEvent(intReset->getExitOprPtr(), getClockMode());
            }
        }

        void makeIncCounterEvent(Node* incNode){
            assert(incNode != nullptr);
            _counter->makeIncEvent(incNode->getExitOprPtr(), getClockMode());
        }
        ...
        Operable*getCounterPtr(){return _counter;}

        void assign() override{
            assert(_counter!= nullptr);
            /**normal start event*/
            for(auto nodeSrc: nodeSrcs){
                _counter->addDependState(nodeSrc.dependNode->getExitOprPtr(),
