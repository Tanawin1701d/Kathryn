    ...
    struct AsmNode : Node {
        ...
        void assignFromStateNode(Operable* holdSignal, Operable* resetSignal){
            ...
            for (int assignIdx = 0; assignIdx < _assignMetas.size(); assignIdx++) {
                ...
                if (assignMeta->asmType == ASM_DIRECT){
                    ...
                    ////// bind node condition with preCondition first\
                    ...
                    condEvent = addLogicWithOutput(condEvent, nodeSrcs[0].dependNode->getStateOperatingPtr(), BITWISE_AND);
                    ...
                    updateWithState->addSubStmt(condEvent, assignMeta->getCurrentEventPtr());
                    assignMeta->setNewEditingEvent(updateWithState);
                    assignMeta->finalUpdate();
                    // auto resultUpEvent = new UpdateEvent({
                    //     condEvent,
                    //     nodeSrcs[0].dependNode->getStateOperatingPtr(), /////// you can not simply use genExitOpr because it is used to colab with reset
                ...
                /** for reg = operator*/
                    ...
                    for (auto nodeSrc: nodeSrcs[0].dependNode->nodeSrcs) {
                        ...
                        condEvent = addLogicWithOutput(condEvent, nodeSrc.dependNode->getExitOprPtr(), BITWISE_AND);
                        ...
                        // auto resultUpEvent = new UpdateEvent({
                        //      condEvent,
                        //      nodeSrc.dependNode->getExitOprPtr(), ///// it is supposed to sensitive to reset already?
                        //      &assignMeta->valueToAssign,
                        //      assignMeta->desSlice,
                        //      _asmPriority,
                        //      _asmId,
                        //      assignMeta->clockMode
                        // });
                        // assignMeta->updateEventsPool.pushBack(resultUpEvent);
                    }
                }else{
                    assert(false);
                }
        ...
        void dryAssign() override{
            ...
            for (int assignIdx = 0; assignIdx < _assignMetas.size(); assignIdx++) {
                ...
                updateWithPureCondState->addSubStmt(nullptr, assignMeta->getCurrentEventPtr());
