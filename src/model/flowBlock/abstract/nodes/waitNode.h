...
namespace kathryn{
    ...
    struct WaitCondNode : Node{
        ...
        void makeUserResetEvent() override{
            if (isThrereIntReset()){
                _condWaitStateReg->makeUserRstEvent(intReset->getExitOprPtr(), getClockMode());
            }
        ...
        void assign() override{
            ...
            for(auto nodeSrc: nodeSrcs){
                _condWaitStateReg->addDependState(nodeSrc.dependNode->getExitOprPtr(), nodeSrc.condition, getClockMode());
            }
            if (isThereHold()){
                _condWaitStateReg->addDependState(getStateOperatingPtr(), holdNode->getExitOprPtr(), getClockMode());
            }
    ...
    struct WaitCycleNode : Node{
        ...
        void makeUserResetEvent() override{
            if(isThrereIntReset()){
                _cycleWaitStateReg->makeUserRstEvent(intReset->getExitOprPtr(), getClockMode());
            }
        ...
        void assign() override{
            ...
            /**normal start event*/
            for(auto nodeSrc: nodeSrcs){
                _cycleWaitStateReg->addDependState(nodeSrc.dependNode->getExitOprPtr(), nodeSrc.condition, getClockMode());
            }
            /** inc event*/
            if (isThereHold()){
                _cycleWaitStateReg->makeIncStateEvent(holdNode->getExitOprPtr(), getClockMode());
            }else{
