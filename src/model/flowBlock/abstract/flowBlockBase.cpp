...
namespace kathryn{
    ...
    FlowBlockBase::~FlowBlockBase(){
        ...
        for (auto subFb: _subBlocks){
            delete subFb;
        }
        for (auto conFb: _conBlocks){
            delete conFb;
        }
        for (auto abandonFb:_abandonedBlocks){
            delete abandonFb;
        }
    ...
    FlowBlockBase* FlowBlockBase::genImplicitSubBlk(FLOW_BLOCK_TYPE defaultType) {
        ...
        FLOW_BLOCK_TYPE nextFbType = ctrl->getTopPatternFlowBlockType();
    ...
    void FlowBlockBase::genSumForceExitNode(std::vector<NodeWrap *> &nws) {
        ...
        for (auto nw : nws){
            _areThereForceExit |= (nw->getForceExitNodePtr() != nullptr);
        }
        ...
        if (_areThereForceExit){
            ...
            for (auto nw : nws){
                if (nw->getForceExitNodePtr() != nullptr){
                    _forceExitNode->addDependNode(nw->getForceExitNodePtr(), nullptr);
                }
            }
    ...
    Operable* FlowBlockBase::genIntSumSignal(bool isAndCond, INT_TYPE intrType){
        ...
        for (int idx = 1; idx < staticCast<int>(targetSignals.size()); idx++){
    ...
    void FlowBlockBase::genHoldNode(){
        ...
        for (int sigId = 1; sigId < staticCast<int>(holdSignals.size()); sigId++){
            holdNode->addLogic(
                holdNode->_value,
                holdSignals[sigId],
                BITWISE_OR
    ...
    std::vector<FlowBlockBase::sortEle> FlowBlockBase::sortSubAndConFbInOrder() {
        ...
        for(int i = 0; i < _subBlocks.size(); i++){
            poolEle.pushBack({_subBlocks[i], _subBlocksOrder[i]});
        }
        for(int i = 0; i < _conBlocks.size(); i++){
            poolEle.pushBack({_conBlocks[i], _conBlocksOrder[i]});
        }
        /**sort array*/
