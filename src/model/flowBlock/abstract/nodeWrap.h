...
//        }
        ...
        Node*getForceExitNodePtr() const {return forceExitNode;}
        ...
        std::string getMdDescribe() override{
            ...
            ret += " use cycle " + std::toString(cycleUsed);

            return ret;
        }

        std::string getMdIdentVal() override{
            return "[nodeWrap @" + std::toString((ull)this) + " ]";
        }
    ...
    struct NodeWrapCycleDet{
        ...
        void addToDet(int cycle){
            samplingVec.pushBack(cycle);
        }

        void addToDet(Node* nd){
            assert(nd != nullptr);
            samplingVec.pushBack(nd->getCycleUsed());
        }
        void addToDet(NodeWrap* nw){
            assert(nw != nullptr);
            samplingVec.pushBack(nw->getCycleUsed());
        }
        ...
        static Node*getMatchNodePtr(const std::vector<Node*>& nds, int cycle){
        ...
        static NodeWrap*getMatchNodeWrapPtr(const std::vector<NodeWrap*>& nws, int cycle){
            assert(cycle != IN_CONSIST_CYCLE_USED);
            for (auto nw: nws){
                if (nw == nullptr){
                    continue;
    ...
    static Node*getMatchNodeFromNdsOrNwsPtr(const std::vector<Node*>& nds,
        ...
        Node* matchedNode = NodeWrapCycleDet::getMatchNodePtr(nds, cycle);

        if (matchedNode != nullptr){
            return matchedNode;
        }
        NodeWrap* matchedNodeWrap = NodeWrapCycleDet::getMatchNodeWrapPtr(nws, cycle);
    ...
    static Node*getAnyNodeFromNdsOrNwsPtr(const std::vector<Node*>& nds,
                                        const std::vector<NodeWrap*>& nws){
        for (auto nd : nds){
            if (nd != nullptr){
                return nd;
