...
namespace kathryn{
    ...
    FlowBlockPar::buildHwComponent() {
        ...
        if (!_basicNodes.empty()){
            ...
            basicStNode->setInternalIdent("parStateReg_" + std::toString(getGlobalId()));
        ...
        for (auto fb : _subBlocks){
            ...
            assert(nw != nullptr);
            nodeWrapOfSubBlock.pushBack(nw);
        }
    ...
    void FlowBlockParAuto::buildSyncNode() {
        int amtBlock = ((basicStNode != nullptr) ? 1 : 0) +
        ...
        if (
            ...
            (cycleUsed == IN_CONSIST_CYCLE_USED) && ///// can't know the longest subblock
             (amtBlock > 1) /** incase amtBlock == 1 we don't have to sync*/
                ){
            /////// syn reg needed
            int synSize = amtBlock;
    ...
    void FlowBlockParAuto::assignExitToRnw() {
        ...
        if (synNode != nullptr){
            ...
            if (cycleUsed >= 0){    /////// can determine cycle
                exitNode = getMatchNodeFromNdsOrNwsPtr({basicStNode},
                                                    nodeWrapOfSubBlock,
                                                    cycleUsed);
            }else{ /////// cannot determine but have only one
                assert(amtBlock == 1); //// in > 1
                exitNode = getAnyNodeFromNdsOrNwsPtr({basicStNode},
                                                  nodeWrapOfSubBlock);
            }
            assert(exitNode != nullptr);
            resultNodeWrap->addExitNode(exitNode);
