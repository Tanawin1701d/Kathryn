...
namespace kathryn{
    ...
    void FlowBlockPick::addSubFlowBlock(FlowBlockBase* subBlock){
        ...
        pickCondBlocks.pushBack(dynamicCast<FlowBlockPickCond*>(subBlock));
    }
    ...
    void FlowBlockPick::buildHwComponent(){
        ...
        for (auto pickCondBlock: pickCondBlocks){
            nodeWrapOfPickCondBlocks.pushBack(pickCondBlock->sumarizeBlock());
        }
        ...
        jointNode->setInternalIdent("jointOfPickNode" + std::toString(getGlobalId()));
        if (isThereIntStart()){
        ...
        for (int sid = 0; sid < nodeWrapOfPickCondBlocks.size(); sid++){
            ...
            ->addDependNodeToAllNode(jointNode,pickCondBlocks[sid]->getConditionPtr());
        ...
        if (reqAutoExit){
            ...
            autoExitNode->setInternalIdent("pickAutoExit" + std::toString(getGlobalId()));
            Operable* allFalse = nullptr;
            for (FlowBlockPickCond* fpc: pickCondBlocks){
                assert(fpc != nullptr);
                if (allFalse == nullptr){
                    allFalse = &(!*fpc->getConditionPtr());
                }else{
                    allFalse = &((*allFalse) & (!*fpc->getConditionPtr()));
                }
            }
        ...
        addSysNode(exitNode);
        exitNode->setInternalIdent("pickExit" + std::toString(getGlobalId()));
        ////// join all exit node
