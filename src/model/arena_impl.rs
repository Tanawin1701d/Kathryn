...
use crate::model::hw_component::common::update_event_ident::{UpdateEventEntry, UpdateEventIdent};
...
impl ModelArena {
    ...
    pub fn reset(&mut self) {
        ...
        self.update_events = ArenaGroup::new();
    }
    ...
    pub fn add_update_event (&mut self, e: UpdateEventEntry)  -> ArenaHandle { self.update_events .insert(e) }
    ...
    pub fn get_update_event_entry(&self, h: ArenaHandle) -> &UpdateEventEntry { self.update_events.get(h) }
    ...
    pub fn get_update_event_entry_mut(&mut self, h: ArenaHandle) -> &mut UpdateEventEntry { self.update_events.get_mut(h) }

    /// Insert a `Box<dyn UpdatingEvent>` into the arena and return its ident.
    pub fn insert_update_event(&mut self, event: Box<dyn UpdatingEvent>) -> UpdateEventIdent {
        let entry = UpdateEventEntry::new(event);
        let h     = self.update_events.insert(entry);
        self.update_events.get(h).get_ident()
    }

    /// Resolve an `UpdateEventIdent` to the concrete event stored in the arena.
    pub fn get_update_event(&self, ident: &UpdateEventIdent) -> &dyn UpdatingEvent {
        let h = *ident.get_arena_handle();
        self.update_events.get(h).get_event()
    }

    pub fn get_update_event_mut(&mut self, ident: &UpdateEventIdent) -> &mut dyn UpdatingEvent {
        let h = *ident.get_arena_handle();
        self.update_events.get_mut(h).get_event_mut()
    }
