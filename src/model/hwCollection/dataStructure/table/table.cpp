...
namespace kathryn{
    ...
    Table::ReducNode Table::createMux(ReducNode& lhs, ReducNode& rhs, Operable& selectLeft, int debugIdx, bool requiredIdx){
        ...
        for (int idx = 0; idx < leftAssMetas.size(); idx++){
            ...
            delete leftAssMeta;
            delete rightAssMeta;
            muxedAssignMeta.pushBack(newAssMeta);
        }
        ...
        if (requiredIdx){
            ...
            selectedIdx = &makeOprWire("reducOpr" + std::toString(debugIdx), lhs.idx->getOperableSlice().getSize());
    ...
    int Table::getNumRow() const{
        return staticCast<int>(_rows.size());
    }
    ...
    void Table::buildRows(SlotMeta& slotMeta, int amtRow, std::string prefixName){
        for (int rowIdx = 0; rowIdx < amtRow; rowIdx++){
            _rows.pushBack(new RegSlot(slotMeta, prefixName + "_" + std::toString(rowIdx)));
        }
    }
    ...
    WireSlot Table::gen_dyn_wire_slotBase(Operable& requiredIdx, bool isOneHotIdx){
        ...
        for(int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            ...
            for (int colIdx = 0; colIdx < resultWireSlot.getNumFieldPtr(); colIdx++){
                eachRowPreCond.pushBack(rowIdxCheckCond);
            }
    ...
    WireSlot Table::gen_dyn_wire_slotBiIdx(Operable& requiredIdx){
        mfAssert(isSufficientBinIdx(requiredIdx), "requiredIdx is not sufficient to get all system");
        return gen_dyn_wire_slotBase(requiredIdx, false);
    }

    WireSlot Table::gen_dyn_wire_slotOHIdx(Operable& requiredIdx){
        mfAssert(isSufficientOhIdx(requiredIdx), "requiredIdx is not sufficient to get all system");
        return gen_dyn_wire_slotBase(requiredIdx, true);
    }
    ...
    void Table::doGlobAsm(Slot& srcSlot, Operable& requiredIdx, ASM_TYPE asmType, bool isOneHotIdx){
        ...
        assert(ctrl != nullptr);
        ctrl->onRegUpdate(asmNode, nullptr);
    ...
    Table& Table::doGlobColAsm(int colIdx, ull assignVal){
        ...
        mfAssert(colIdx < getMeta().getNumFieldPtr(), "colIdx is out of range");
        ////// gen the value
        ...
        for (int desRowIdx = 0; desRowIdx < getNumRow(); desRowIdx++){
            ...
            allRowCollector.pushBack(asmMeta);
        }
        ////// create asmNode
        auto* asmNode = new AsmNode(allRowCollector);
    ...
    Table& Table::doGlobColAsm(const std::string& colName, ull assignVal){
        int targetColIdx = _meta.getIdxPtr(colName);
    ...
    WireSlot Table::doReducNoIdx(const std::function<Operable&(WireSlot& lhs, Operable* lidx,
        ...
        for (int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            initReducNodes.pushBack({new WireSlot(*staticCast<Slot*>(_rows[rowIdx]), "initReduc"), nullptr});
        }
    ...
    std::pair<WireSlot, Operable&> Table::doReducBinIdx(const std::function<Operable&(WireSlot& lhs, Operable* lidx,
        ...
        std::vector<ReducNode> initReducNodes;
        for (int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            Val* idxVal = &makeOprVal("initBinIdxOpr" + std::toString(rowIdx), getSufficientIdxSize(false), rowIdx);
            initReducNodes.pushBack({new WireSlot(*staticCast<Slot*>(_rows[rowIdx]), "initReducBin"), idxVal});
        }
    ...
    std::pair<WireSlot, OH> Table::doReducOhIdx(const std::function<Operable&(WireSlot& lhs, Operable* lidx,
        ...
        std::vector<ReducNode> initReducNodes;
        for (int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            Val* idxVal = &makeOprVal("initOhIdxOpr" + std::toString(rowIdx), getSufficientIdxSize(true), ((ull) 1) << rowIdx);
            initReducNodes.pushBack({new WireSlot(*staticCast<Slot*>(_rows[rowIdx]), "initReducOh"), idxVal});
        }
        ReducNode finalNode = doReduceBase(initReducNodes, cusLogic, true);
        WireSlot result(*finalNode.slot);
        Operable& resultIdx = *finalNode.idx;
    ...
    WireSlot* Table::augmentForOrderedSearch(int rowIdx,
        ...
        auto* result =
        new WireSlot(*staticCast<Slot*>(_rows[rowIdx]), "augOldest_" + std::toString(rowIdx));
        ////// do augment wire
    ...
    std::pair<WireSlot, Operable&>
        ...
        for (int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            ...
            initReducNodes.pushBack({augSlot, idxVal});
        }
        ...
        return {result(0, getMeta().getNumFieldPtr()), resultIdx};
    ...
    std::pair<WireSlot, OH>
        ...
        finalNode.destroy();
        ///// we have to delete the augmented filed
        return {result(0, getMeta().getNumFieldPtr()-2), OH(resultIdx)};
    ...
    RegSlot& Table::operator () (int idx){
        mfAssert(isValidIdx(idx), "index out of range to get " + std::toString(idx));
        return *_rows[idx];
    }
    ...
    Table Table::sliceByCol(int start, int end){
        ...
        for (int rowIdx = 0; rowIdx < getNumRow(); rowIdx++){
            ...
            newRows.pushBack(new RegSlot(newRegSlot));
        }
    ...
    Table Table::sliceByCol(const std::string& startField, const std::string& endField){
        int startIdx = getMeta().getIdxPtr(startField);
        int endIdx   = getMeta().getIdxPtr(endField) + 1;
    ...
    Table Table::sliceByCol(const std::vector<std::string>& fieldNames){
        std::vector<int> fieldIdxs = getMeta().getIdxPtrs(fieldNames);
    ...
    TableSliceAgent Table::operator[] (OH ohIdx){
        isSufficientOhIdx(ohIdx.getIdxPtr());
        return TableSliceAgent(this, ohIdx.getIdxPtr(), true);
    }
