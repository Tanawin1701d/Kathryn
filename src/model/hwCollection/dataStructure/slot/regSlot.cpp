...
namespace kathryn{
    ...
    RegSlotDynSliceAgent& RegSlotDynSliceAgent::operator <<=(Operable& rhsOpr){
        _masterSlot.doBlockAsm   (rhsOpr, _required_idx, ASM_DIRECT, _isOH);
        return *this;
    }

    RegSlotDynSliceAgent& RegSlotDynSliceAgent::operator <<=(ull rhsVal){
        _masterSlot.doBlockAsm   (rhsVal, _required_idx, ASM_DIRECT, _isOH);
        return *this;
    }

    RegSlotDynSliceAgent& RegSlotDynSliceAgent::operator =(Operable& rhsOpr){
        _masterSlot.doNonBlockAsm(rhsOpr, _required_idx, ASM_EQ_DEPNODE, _isOH);
        return *this;
    }

    RegSlotDynSliceAgent& RegSlotDynSliceAgent::operator =(ull rhsVal){
        _masterSlot.doNonBlockAsm(rhsVal, _required_idx, ASM_EQ_DEPNODE, _isOH);
        return *this;
    }
    ...
    RegSlot::RegSlot(const SlotMeta& slotMeta,
        ...
        Slot(slotMeta){
            ...
            mfAssert(slotMeta.getNumFieldPtr() == regs.size(), "fieldMetas size not match with regs size");
            _regs = regs;
            for(Reg* reg: _regs){
                _hwFieldMetas.pushBack({reg, reg});
            }
        }
    ...
    void RegSlot::initHwStructure(const std::string& prefixName){
        for(int idx = 0; idx < _meta.getNumFieldPtr(); idx++){
            ...
            Reg* newReg = &mOprReg(prefixName +
                "colIdx_" + std::toString(idx) +
                "_" + fieldMeta._name,
                fieldMeta._size);
            _regs.pushBack(newReg);
            _hwFieldMetas.pushBack({newReg, newReg});
        }
    }
    ...
    void RegSlot::makeResetEvent(int colIdx, ull resetValue, CLOCK_MODE cm){
        mfAssert(isValidIdx(colIdx), "index out of range to get " + std::toString(colIdx));
    ...
    RegSlot RegSlot::operator() (const std::string& startField,
        ...
        int idx    = getIdxPtr(startField);
        int endIdx = getIdxPtr(endField) + 1;
    ...
    RegSlot RegSlot::operator() (const std::vector<std::string>& fieldNames){
        std::vector<int> fieldIdxs = getIdxPtrs(fieldNames);
