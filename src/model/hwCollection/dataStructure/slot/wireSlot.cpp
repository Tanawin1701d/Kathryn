...
namespace kathryn{
    ...
    WireSlotDynSliceAgent& WireSlotDynSliceAgent::operator =(Operable& rhsOpr){
        _masterSlot.doNonBlockAsm(rhsOpr, _required_idx, ASM_DIRECT, _isOH);
        return *this;
    }

    WireSlotDynSliceAgent& WireSlotDynSliceAgent::operator =(ull rhsVal){
        _masterSlot.doNonBlockAsm(rhsVal, _required_idx, ASM_DIRECT, _isOH);
        return *this;
    }
    ...
    WireSlot::WireSlot(const SlotMeta& slotMeta,
        ...
        Slot(slotMeta){
            ...
            mfAssert(slotMeta.getNumFieldPtr() == wires.size(), "fieldMetas size not match with wires size");
            _wires = wires;
            for(Wire* wire: _wires){
                _hwFieldMetas.pushBack({wire, wire});
            }
        }
    ...
    void WireSlot::initHwStructure(const std::string& prefixName){
        ...
        for(int idx = 0; idx < _meta.getNumFieldPtr(); idx++){
            ...
            _wires.pushBack(newWire);
            _hwFieldMetas.pushBack({newWire, newWire});
        }
