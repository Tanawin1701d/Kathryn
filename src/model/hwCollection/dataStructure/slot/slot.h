...
namespace kathryn{
    ...
    public:
        ...
        HwFieldMeta& hwFieldRefAt(int idx){
            mfAssert(isValidIdx(idx), "get hw Field at " + std::toString(idx) + " out of range");
        ...
        void addHwFieldMeta(HwFieldMeta hwFieldMeta){
            _hwFieldMetas.pushBack(std::move(hwFieldMeta));
        }
        ...
        int getIdxPtr(std::string fieldName) const{
            return _meta.getIdxPtr(std::move(fieldName));
        }

        std::vector<int> getIdxPtrs(std::vector<std::string> fieldNames) const{
            return _meta.getIdxPtrs(std::move(fieldNames));
        }

        int getNumFieldPtr() const{
            return _meta.getNumFieldPtr();
        }
        ...
        std::vector<AssignMeta*> genAssignMetaForAll(const Slot& srcSlot, ASM_TYPE asmType) const{
            assert(getNumFieldPtr() == srcSlot.getNumFieldPtr());
            std::vector<AssignMeta*> resultCollector;
            for (int desIdx = 0; desIdx < srcSlot.getNumFieldPtr(); desIdx++){
        ...
        std::vector<AssignMeta*> genAssignMetaForAll(std::vector<Operable*> srcOperables, ASM_TYPE asmType) const{
            assert(getNumFieldPtr() == srcOperables.size());
            std::vector<AssignMeta*> resultCollector;
            for (int desIdx = 0; desIdx < getNumFieldPtr(); desIdx++){
                auto [desOpr, desAsb] = hwFieldAt(desIdx);
        ...
        virtual void doBlockAsm(const Slot& rhs, const std::vector<std::string>& exceptNames, ASM_TYPE asmType){
            std::vector<int> excpetIdxs = getIdxPtrs(exceptNames);
    ...
    public:
        ...
        Operable& v(){
            ...
            for (int idx = 0; idx < _masterSlot.getNumFieldPtr(); idx++){
                ...
                if (!isUsedAsDef){
                    if(_isOH){
                        activateCond = &_required_idx.sl(idx);
                    }else{
                        activateCond = &(_required_idx == idx);
                    }
