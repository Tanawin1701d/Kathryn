...
namespace kathryn{
    ...
    void ModuleGen::startWriteFile(FileWriterBase* fileWriter,
        ...
        for (ModuleGen* subMdGen: _subModulePool){
            ...
            for (LogicGenBase* mdOutput: subMdGen->_wirePoolWithOutputMarker){
                ...
                subModuleOutputRepresent.pushBack(mdOutput);
            }
            ////////// input
            ...
            for (LogicGenBase* mdInput: subMdGen->_wirePoolWithInputMarker){
                ...
                subModuleInputRepresent.pushBack(mdInput);
            }
        }
        ///////////////////////////////////////////////////////////
        ...
        ioVec.emplaceBack("input wire clk");
    ...
    std::vector<std::string> ModuleGen::getIoDec(){
        ...
        for (LogicGenBase* lgb: _wirePoolWithInputMarker                ){result.pushBack(lgb->decIo());}
        for (LogicGenBase* lgb: _wirePoolWithOutputMarker               ){result.pushBack(lgb->decIo());}
        for (LogicGenBase* lgb: _genWirePools[WIRE_AUTO_GEN_INPUT]      ){result.pushBack(lgb->decIo());}
        for (LogicGenBase* lgb: _genWirePools[WIRE_AUTO_GEN_OUTPUT]     ){result.pushBack(lgb->decIo());}
        for (LogicGenBase* lgb: _genWirePools[WIRE_AUTO_GEN_GLOB_INPUT] ){result.pushBack(lgb->decIo());}
        for (LogicGenBase* lgb: _genWirePools[WIRE_AUTO_GEN_GLOB_OUTPUT]){result.pushBack(lgb->decIo());}
    ...
    std::vector<std::string> ModuleGen::getParamDec(){
        ...
        std::vector<std::string> result;
        for (LogicGenBase* lgb: _pmValPool){result.pushBack(lgb->decParamVal());}
        return result;
    ...
    std::string ModuleGen::getSubModuleDec(ModuleGen* subMdGen){
        ...
        if (!subMdGen->_pmValPool.empty()){
            ...
            for (LogicGenBase* lgb: subMdGen->_pmValPool){
                ioParamVals.pushBack(lgb->decOp());
            }
