...
namespace kathryn{
    ...
    void ModuleGen::startInitEle(){
        ...
        if (_master->getParentPtr() == nullptr){
            /// current module is top module
            depthFromGlobalModule = 0;
        }else{
            depthFromGlobalModule = _master->getParentPtr()
                                    ->getModuleGenPtr()->getDept() + 1;
        }
        ...
        for (Module* subModule: _master->getUserSubModules()){
            ...
            subModule->getModuleGenPtr()->startInitEle(); //// init sub module element
            _subModulePool.pushBack(subModule->getModuleGenPtr()); //// add to this module pool
        }
        ...
        for (Wire* wire: _master->getUserWires()){
            ...
            switch (wire->getMarker()){
                case WMT_INPUT_MD:{
                    _wirePoolWithInputMarker.pushBack(wire->getLogicGenPtr());
                    break;
                }
                case WMT_OUTPUT_MD:{
                    _wirePoolWithOutputMarker.pushBack(wire->getLogicGenPtr());
                    break;
                }
                default:{
                    _wirePool.pushBack(wire->getLogicGenPtr());
                    break;
                }
            }
        }
