...
namespace kathryn{
    ...
    public:
        ...
        std::unorderedMap<Operable*, int> _genWireMaps[WIRE_AUTO_GEN_CNT];
        ...
        Module*getMasterModulePtr() const {return _master;}
        ...
        void recruitLogicGenBase(LogicGenBaseVec& des,
                                 std::vector<T*>& srcs){
            for(T* src: srcs){
                LogicGenBase* logicGenBase = src->getLogicGenPtr();
                assert(logicGenBase != nullptr);
                des.pushBack(logicGenBase);
            }
        }
    ...
    public:
        ...
        virtual ModuleGen*getModuleGenPtr() = 0;
