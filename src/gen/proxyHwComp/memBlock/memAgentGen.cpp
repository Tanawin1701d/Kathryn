...
namespace kathryn{
    ...
    void MemEleholderGen::routeDep(){
        //////////// route from indexer
        routedIndexer =
            _mdGenMaster->routeSrcOprToThisModule(_master->getIndexerPtr());
    ...
    std::string MemEleholderGen::decVariable(){
        ...
        ///////// read mode
        return "wire [" + std::toString(sl.stop-1) +
            ": 0] " + getOpr() + ";";
    ...
    std::string MemEleholderGen::decOp(){
        ...
        std::string   masterMemBlkOpr = _master->getMasterMemBlkPtr()
                                        ->getLogicGenPtr()->getOpr();
