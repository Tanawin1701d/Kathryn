...
namespace kathryn{
    ...
    void GenController::initEle(){
        ...
        _masterModuleGen = _masterModule->getModuleGenPtr();
    ...
    void GenController::startSynthesis(){
        ...
        int compileComd = system(compileComand.cStr());
    ...
    GenController*getGenControllerPtr(){
    ...
    void GenController::initGlobEle(bool isInput){
        ...
        for(WireMarker* srcToBeGlobIo: getGlobPool(isInput)){
            ...
            Operable* originOpr = srcToBeGlobIo->getOprFromGlobIoPtr();
            ...
            if(isInput){
                ...
                srcToBeGlobIo->getAsbFromWireMarkerPtr()->addUpdateMeta(connectEvent);
            }else{
                inputIo.connectTo(originOpr, false); /////// it may needs to be route
            }
            desSaveIo.pushBack(&inputIo);
        }
