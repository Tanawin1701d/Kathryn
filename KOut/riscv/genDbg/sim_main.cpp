...
int main(int argc, char** argv ){
    ...
    for (int testCaseIdx = 0; testCaseIdx < AMT_TEST_CASE; testCaseIdx++){
        ...
        while(true){
            ...
            // if (core->st_wb  == 1){
            //     std::cout << curCycleUsed  << 
                
            //       " writeIdx "<< static_cast<int>(core->writeIdx) << " have value: " << core->writeData  << std::endl;
            // }
            ...
            // std::cout
            // << " PC: "         << std::hex  << static_cast<int>(core->curPc) << std::dec
            // << "FETCH:"        << std::hex  << static_cast<int>(core->st_fetch) 
            // << " DECODE:"      << std::hex  << static_cast<int>(core->st_decode)
            // << " EXECUTE: rg"  << std::hex  << static_cast<int>(core->st_regAccess) 
            // << " EXECUTE: alu" << std::hex  << static_cast<int>(core->st_alu) 
            // << " WB: "         << std::hex  << static_cast<int>(core->st_wb) << std::endl;

            curCycleUsed++;

            // if (curCycleUsed > 90000){
            //      std::cout << " PC: " << static_cast<int>(core->curPc) << std::endl;
            
            // }

            // if (curCycleUsed > 1000){
            //     break;
            // }
